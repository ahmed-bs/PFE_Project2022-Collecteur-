...
export class ListeOperationComponent implements OnInit {
  ...
  deleteOperation(id: number) {
    ...
    let confirmation = confirm(
      "Êtes-vous sûr de supprimer l'operation où son id est egale à : " +
        id +
        ' ??'
    );
    if (confirmation)
        ...
        this.operationService.getNbOpTankTotal(id).subscribe((b) => {
          ...
          this.operationService.getNbOpTank(id).subscribe((c) => {
            ...
            if (this.p <= this.q && this.test1 == this.test2) {
              ...
              this.Toast[0] = 'Failed';
              this.Toast[1] =
                'Vous avez déja utilisé la quantité de laits inserée dans les tanks affectées a cette opération !!';
              this.showToast();
            }
          });
        });
